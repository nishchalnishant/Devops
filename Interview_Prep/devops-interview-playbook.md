...
│   ├── 1. Fundamentals: Linux, networking, cloud, CI/CD, containers, observability — when + why not just what
│   ├── 2. Systems Thinking: dependencies, blast radius, downstream effects, safe mitigation
│   ├── 3. Troubleshooting Depth: metrics → logs → commands → rollback (no premature root cause guessing)
│   ├── 4. Automation Mindset: repeated tasks become scripts, pipelines, modules, runbooks
│   └── 5. Communication: structured answers (check → why → expect → mitigate → prevent)
├── Strong Answer Framework (for most questions and scenarios)
│   ├── 1. Clarify symptom and impact
│   ├── 2. Check for recent deployment or config change
│   ├── 3. Start with user-facing signals (latency, errors, availability, saturation)
│   ├── 4. Narrow failing layer: app / container / node / network / database / pipeline
│   ├── 5. Run smallest commands to confirm or reject hypothesis
│   └── 6. Stabilize first → optimize → document long-term fix
├── What You Should Know By Topic
│   ├── Git: branching, PRs, merge vs rebase, revert vs reset, protected branches
│   ├── Linux: processes, signals, permissions, systemd, disk/memory inspection, safe scripting
│   ├── Networking: TCP/UDP, DNS, CIDR, routing, NAT, TLS, L4/L7 LB, ss/curl/dig/tcpdump
│   ├── Cloud: compute, storage, networking, IAM, autoscaling, HA, DR, managed service trade-offs
│   ├── CI/CD: build-test-scan-package-publish-deploy-verify-rollback, artifact immutability, secrets
│   ├── Docker + K8s: layers, registries, Deployment, Service, Ingress, probes, HPA, common failures
│   ├── Terraform + Ansible: state, backends, modules, drift, for_each vs count, CM purpose difference
│   ├── Observability + SRE: Four Golden Signals, SLI/SLO/error budget, alert fatigue prevention
│   └── Security: secrets management, least privilege, image scanning, SBOMs, policy as code
├── Must-Know Commands
│   ├── Git: status, log --graph, diff, revert, fetch --prune
│   ├── Linux: top/htop, ps aux, journalctl, systemctl, df -h, free -m, ss -tulpn, lsof -i
│   ├── Networking: curl -v, dig, nslookup, ip addr/route, traceroute/mtr, tcpdump
│   ├── Docker: ps, logs, exec -it sh, inspect, stats
│   ├── Kubernetes: get pods -A, describe pod, logs --previous, get events --sort-by=.lastTimestamp, top pod, rollout status/undo
│   └── Terraform: init, plan, show, state list, force-unlock; Ansible: ping, playbook --check
├── High-Value Scenarios To Practice
│   ├── CrashLoopBackOff: describe → logs --previous → config/secret check → OOM → probe → rollout undo
│   ├── ImagePullBackOff: tag → auth → imagePullSecrets → registry reachability → DNS
│   ├── Pending / FailedScheduling: requests → taints → affinity → PVC zone → quota
│   ├── High Latency After Scaling: CPU throttling → connection pool → downstream → DNS → conntrack
│   ├── Terraform Recreate Critical Resource: stop → plan inspect → ForceNew → state/import → prevent_destroy
│   └── Monitoring No Data: /targets page → /metrics reachability → exporter → ServiceMonitor → NetworkPolicy
├── Design Trade-Offs To Explain
│   ├── Merge vs rebase
...
│   ├── Lead with impact, not guesswork
│   ├── Mention rollback and mitigation before perfect root cause
│   ├── Call out trade-offs rather than declaring one tool always best
│   ├── Validate after the fix
│   └── Explain how to prevent recurrence with automation, guardrails, or observability
├── Behavioral Interview Prep (STAR themes)
...
│   ├── A disagreement you resolved with devs, QA, or security
│   └── A time you reduced MTTR, cost, failure rate, or deployment time
└── Final Revision Checklist
    ├── Can explain request path from browser to backend and database
    ├── Can describe debugging a broken K8s deployment
    ├── Can explain Git rollback without confusing reset and revert
    ├── Can explain Terraform state, backends, locking, and drift
    ├── Can explain probes, Services, Ingress, HPA, and pod failure states
    ├── Can describe CI/CD safety controls (immutability, approvals, rollback)
    ├── Can explain metrics, logs, traces, SLOs, and alert fatigue
    └── Can talk through at least two real troubleshooting stories
```

## First Principles

- **Why structure answers before content?** Unstructured answers signal panic, not competence. The answer framework (scope → change check → signals → narrow → command → stabilize) works because it mirrors how senior engineers actually think in incidents.
- **Why troubleshooting depth over tool memorization?** Tools change. The mental model of narrowing from symptom to layer to command to hypothesis stays constant. Interviewers test whether you have that model, not whether you remember every flag.
- **Why say "rollback" before "root cause"?** In a live production failure, user impact is the priority. An engineer who stops the bleeding first and investigates after demonstrates judgment. An engineer who investigates while users suffer demonstrates poor prioritization.
- **Why behavioral STAR themes alongside technical questions?** Senior roles require influence without authority, cross-team communication, and project ownership. Behavioral questions test evidence of those skills. Technical competence alone is insufficient.
- **Why a final checklist?** The checklist converts diffuse anxiety into specific gaps. Each item is either ready or not. Each gap is a concrete study target, not a vague worry about "knowing enough."
