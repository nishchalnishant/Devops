...
3. [Linux Namespaces — Isolation Internals](#3-linux-namespaces--isolation-internals)
4. [cgroups — Resource Control](#4-cgroups--resource-control)
5. [Union Filesystems — overlay2 and devicemapper](#5-union-filesystems--overlay2-and-devicemapper)
6. [OCI Specification](#6-oci-specification)
7. [Container Runtimes — runc, containerd, CRI-O](#7-container-runtimes--runc-containerd-cri-o)
8. [Image Layering and Build Cache](#8-image-layering-and-build-cache)
9. [Multi-Stage Builds](#9-multi-stage-builds)
10. [Docker Networking Deep Dive](#10-docker-networking-deep-dive)
11. [Volumes and Bind Mounts](#11-volumes-and-bind-mounts)
...
| Isolation | Namespace + cgroup | Full hypervisor isolation |
| Start time | Milliseconds | Seconds to minutes |
| Disk footprint | MBs | GBs |
| Memory overhead | Near zero | Fixed guest OS overhead |
| Security boundary | Weaker (shared kernel) | Stronger (separate kernel) |
| Portability | Image (tens of MB) | Disk image (GBs) |

Containers are **isolated processes** running in user space on the host kernel. The isolation comes from Linux primitives — namespaces for isolation, cgroups for resource limits. There is no hypervisor call path, so syscalls go directly to the host kernel (this is also the attack surface).
...
┌──────────────────────────────────────────────────────┐
│  docker CLI (client)                                 │
│  REST API calls → unix:///var/run/docker.sock        │
└───────────────────────┬──────────────────────────────┘
                        │ REST API
┌───────────────────────▼──────────────────────────────┐
...
│  - Network management (bridge, overlay, macvlan)     │
│  - Volume management                                 │
└───────────────────────┬──────────────────────────────┘
                        │ gRPC (containerd API)
┌───────────────────────▼──────────────────────────────┐
...
└───────────────────────┬──────────────────────────────┘
                        │ OCI runtime spec
┌───────────────────────▼──────────────────────────────┐
│  containerd-shim + runc (low-level runtime)          │
│  - Creates namespaces and cgroups                    │
│  - Execs the container process                       │
│  - shim keeps container running if containerd dies   │
└───────────────────────┬──────────────────────────────┘
                        │ syscalls
...
**Key design points:**
- The shim (`containerd-shim-runc-v2`) is the parent process of PID 1 inside the container. It allows `containerd` to be restarted without killing containers.
...
A namespace wraps a global system resource in an abstraction so that processes within a namespace see their own isolated instance of the resource.

| Namespace | Kernel flag | Isolates |
|-----------|-------------|----------|
| `pid` | `CLONE_NEWPID` | Process ID tree — PID 1 inside container is not host PID 1 |
| `net` | `CLONE_NEWNET` | Network stack: interfaces, routing table, iptables, sockets |
...
| `time` | `CLONE_NEWTIME` | Boot and monotonic clock offsets (Linux 5.6+) |

**How runc creates namespaces:**

```c
// Simplified — runc calls clone(2) with namespace flags
clone(CLONE_NEWPID | CLONE_NEWNET | CLONE_NEWNS | CLONE_NEWUTS | CLONE_NEWIPC, ...)
```

**Inspecting container namespaces from the host:**
...
cgroups (control groups) limit, account for, and isolate resource usage. namespaces provide **isolation**; cgroups provide **enforcement**.

### cgroup v1 vs v2

| Feature | cgroup v1 | cgroup v2 |
|---------|-----------|-----------|
| Hierarchy | Separate trees per subsystem | Unified single hierarchy |
| Thread granularity | Per-process | Per-thread (threaded mode) |
| Memory PSI | No | Yes (pressure stall info) |
| Rootless support | Limited | Full |
| Kernel | All modern | 4.5+ (unified); mandatory 5.2+ |

cgroup v2 is the default on most modern Linux distributions. Docker supports both.

### Key subsystems

| Subsystem | What it controls |
|-----------|-----------------|
| `memory` | RAM, swap, OOM killer threshold |
| `cpu` / `cpuacct` | CPU shares, CFS quota and period |
| `cpuset` | Pin to specific CPU cores and NUMA nodes |
| `blkio` / `io` | Block I/O weight and throttle (read/write BPS, IOPS) |
| `pids` | Maximum number of PIDs (prevents fork bombs) |
| `net_cls` | Tag network packets for traffic shaping |
| `devices` | Allowlist/denylist device access |
| `hugetlb` | Huge page limits |

### Docker resource flags → cgroup mappings

```bash
docker run \
  -m 512m                          # memory.limit_in_bytes = 536870912
  --memory-swap 1g                 # memory.memsw.limit_in_bytes
  --memory-reservation 256m        # memory.soft_limit_in_bytes
  --cpus="1.5"                     # cpu.cfs_quota_us=150000, period=100000
  --cpu-shares=512                 # cpu.shares (relative weight, default 1024)
...
# docker stats reads these in real time
docker stats --no-stream mycontainer
```

**OOM behavior:**

When a container exceeds its memory limit, the kernel OOM killer sends `SIGKILL` to the container's PID 1. Docker records this as `OOMKilled: true` in the container state. You can observe it:
...
overlay2 is Docker's default storage driver on Linux with kernel 4.0+. It is backed by the kernel OverlayFS filesystem.

```
Container view (merged):
┌─────────────────────────────────┐
│  /var/lib/docker/overlay2/      │
│  <layer-id>/merged/             │  ← unified view shown to container
└────────────┬──────────┬─────────┘
             │          │
    ┌────────▼───┐  ┌───▼────────┐
    │ upperdir   │  │ lowerdir   │
...
- **Copy-on-Write (CoW):** When a container modifies a file from a lower (image) layer, OverlayFS copies the entire file to `upperdir` first, then modifies it. The lower layer is never touched.
- **Layer sharing:** Multiple containers from the same image share the same lower dirs. No duplication on disk.
- **Deletion:** A whiteout file (`.wh.<filename>`) in the upper layer masks the file in lower layers.

**Inspect overlay paths:**
```bash
docker inspect mycontainer | jq '.[0].GraphDriver.Data'
# Returns LowerDir, UpperDir, MergedDir, WorkDir
```

**overlay2 limitations:**
- Rename operations across directories are not atomic (kernel 4.18+ improves this).
- Does not support NFS or certain network filesystems as lower directories.
- Inodes can be exhausted with many layers on older kernels.

### devicemapper (legacy)

Uses Linux Device Mapper's thin-provisioning target. Each image/container gets a thin provisioned block device. Copy-on-Write is at block level (not file level), so it can be more efficient for large file modifications. However, it requires more complex setup (loop or direct-lvm mode) and is deprecated in favor of overlay2.
...
|-----------|-------------|
| **Image Index** | Multi-platform manifest list (pointers to platform-specific manifests) |
| **Image Manifest** | JSON document listing config blob and layer blobs |
| **Config blob** | JSON: container config (Cmd, Env, ExposedPorts, etc.) + layer diff IDs |
| **Layer blobs** | gzip-compressed tar archives (one per Dockerfile instruction that changes filesystem) |

```json
// Simplified image manifest
{
  "schemaVersion": 2,
  ...
  "layers": [
    { "mediaType": "application/vnd.oci.image.layer.v1.tar+gzip",
      "digest": "sha256:def456...", "size": 27098913 },
    { "mediaType": "application/vnd.oci.image.layer.v1.tar+gzip",
      "digest": "sha256:ghi789...", "size": 23208828 }
  ]
}
```

Layers are **content-addressed** by their SHA256 digest. If two images share a layer with the same digest, the registry and local store keep only one copy.

### OCI Runtime Spec

Defines `config.json` — the bundle that a low-level runtime (`runc`) consumes to create a container. It specifies:

- **`root`** — path to container root filesystem
- **`process`** — entrypoint, args, env, uid/gid, capabilities, rlimits
- **`mounts`** — filesystems to mount (proc, dev, sys, tmpfs, bind mounts)
- **`linux`** — namespaces, cgroups, seccomp, apparmor, selinux
- **`hooks`** — prestart, createRuntime, poststart, poststop lifecycle hooks

```bash
# Inspect the OCI config for a running container
docker inspect mycontainer | jq '.[0].HostConfig'

# Generate config.json manually (for learning)
...
containerd-shim-runc-v2
    ...
    │  fork/exec
    ▼
runc  ← low-level runtime (creates namespaces, cgroups, execs PID 1)
    │
    ▼
...
- Reference implementation of OCI Runtime Spec, written in Go
- Creates Linux namespaces via `clone(2)` / `unshare(2)`
- Configures cgroups, mounts, capabilities, seccomp, AppArmor
- Execs the container's init process (`execve(2)`)
- After exec, runc exits — the container process is orphaned to the shim

```bash
# runc can be used directly (useful for debugging)
runc run mycontainer   # from a bundle directory with config.json + rootfs/
runc list
...
- Parent of container PID 1 — collects exit status
- Forwards API calls (exec, kill, resize PTY) to the container
- Allows `containerd` to be restarted without killing containers
- Implements `containerd-shim-runc-v2` gRPC API

### containerd

- High-level runtime, Docker's core dependency
- Manages: image pull/push/store, snapshotter (overlay2), container execution via shims
- Implements CRI for Kubernetes (via `cri` plugin)
- Written in Go, CNCF project
...
# crictl works against CRI (useful in Kubernetes nodes)
crictl ps
crictl images
...
- Lightweight CRI implementation specifically for Kubernetes
- No Docker dependency, no containerd — uses `conmon` (container monitor) instead of containerd-shim
- Uses runc (or any OCI-compliant runtime) for execution
- Preferred in OpenShift / bare-metal Kubernetes setups

### Secure Sandboxed Runtimes

| Runtime | Mechanism | Use case |
|---------|-----------|----------|
| `runc` | Namespaces + cgroups | Trusted workloads |
| `gVisor` (runsc) | User-space kernel (Go), intercepts syscalls | Multi-tenant SaaS, untrusted code |
| `Kata Containers` | Lightweight VM (QEMU/KVM) per container | Strong isolation, near-VM security |
| `nydus` / `eStargz` | Lazy image loading via FUSE/OverlayFS | Large images, fast cold start |

**Deploy gVisor in Kubernetes:**
```yaml
...
Each Dockerfile instruction that modifies the filesystem creates a new layer (a gzipped tar archive of the filesystem diff). Read-only metadata instructions (`ENV`, `LABEL`, `EXPOSE`, `USER`, `WORKDIR`, `ARG`, `CMD`, `ENTRYPOINT`) create a new **config** entry but not a new filesystem layer.

```
FROM ubuntu:22.04          → Layer 0 (base): ~29 MB
RUN apt-get update \       → Layer 1: package lists delta
    && apt-get install nginx
COPY nginx.conf /etc/...   → Layer 2: one config file
EXPOSE 80                  → metadata only (no layer)
CMD ["nginx", "-g", ...]   → metadata only (no layer)
```

### Build cache mechanics

Docker computes a **cache key** for each instruction:

1. Parent layer digest (all previous layers must match)
2. For `RUN`: the instruction string itself
3. For `COPY`/`ADD`: the checksum of the files being copied

If all inputs match a cached layer, Docker reuses that layer without executing the instruction.

**Cache invalidation cascades**: invalidating one layer invalidates all subsequent layers. This is why layer ordering is a critical optimization:

```dockerfile
# BAD — source code changes invalidate pip install
...
### BuildKit cache differences

BuildKit does not use the classic Docker build cache algorithm. It uses:

- **Content-addressable cache** backed by the BuildKit snapshotter
- **Cache mounts** (`--mount=type=cache`) that persist across builds, independent of layer cache
- **Registry-based cache** (`--cache-from` / `--cache-to`) for sharing cache between CI runners

```bash
# Export build cache to registry
docker buildx build \
...
Multi-stage builds use multiple `FROM` instructions in a single Dockerfile. Only the final stage is included in the output image.
...
**Result:** The Go toolchain (400+ MB) never enters the final image. The output is the binary + distroless base (~3 MB).
...
```bash
# Build only up to test stage in CI
...
Host network stack
  │
  ├── eth0 (host physical interface) — external traffic
  │
  └── docker0 (virtual bridge, 172.17.0.1/16)
        │
        ├── veth0abc (host end of pair) ─── eth0 → Container A (172.17.0.2)
        └── veth1def (host end of pair) ─── eth0 → Container B (172.17.0.3)
```

- Docker creates `docker0` bridge at daemon startup
- Each container gets a `veth` (virtual ethernet) pair
- One end of the veth pair is placed in the container's `net` namespace as `eth0`
- The other end is attached to `docker0` on the host
- Docker manages iptables MASQUERADE rules for outbound NAT
- Port publishing (`-p 8080:80`) adds a DNAT rule: host:8080 → container:80

```bash
# Inspect iptables rules Docker creates
...
**Default `docker0` bridge limitation:** container name DNS resolution does not work. Use custom bridges.
...
# app can reach nginx by hostname: http://web:80
```

Custom bridges have an embedded DNS server (127.0.0.11) that resolves container names and network aliases.

### Host

```
Container shares host's net namespace — no isolation, no NAT.
Container sees host's eth0, all ports are host ports.
```

```bash
docker run --network host nginx
# nginx binds to host port 80 directly — no port mapping needed/possible
```

**Use case:** High-throughput applications where NAT overhead is significant. Not available on Docker Desktop (macOS/Windows) — the Linux VM intercepts.

### None

```bash
docker run --network none alpine
# Only loopback (lo) — no external connectivity
```

**Use case:** Batch processing jobs that must not make network calls, security-sensitive workloads.

### Overlay (Swarm / multi-host)

Overlay networks use VXLAN encapsulation (UDP port 4789) to tunnel packets between Docker daemon hosts.
...
Container A (10.0.0.2)        Container B (10.0.0.3)
    │                                  │
    └── VXLAN packet ──────────────────┘
        (outer: Host A eth0 → Host B eth0)
        (inner: 10.0.0.2 → 10.0.0.3)
```

```bash
# Requires swarm mode or --attachable flag
docker swarm init
...
Assigns a real MAC address to each container, making it appear as a physical NIC on the LAN. The container gets an IP from the physical network's subnet via DHCP or static assignment.
...
docker run --network mymacvlan --ip 192.168.1.130 --name db postgres
```

**Use case:** Legacy apps expecting to be directly on the physical network; IoT gateways.

**Limitation:** The host cannot communicate with macvlan containers by default (different MAC addresses on same parent). Create a macvlan interface on the host to solve this.

### ipvlan

Similar to macvlan but containers share the host's MAC address. Two modes:
- `L2`: Same broadcast domain as parent, unique IPs
- `L3`: No ARP, routing based — better for large deployments

### DNS and Service Discovery

Docker's embedded DNS server listens on `127.0.0.11:53` inside containers on custom networks. It resolves:

- Container names → container IP
- Network aliases (`--network-alias`) → container IP
- Service names (Swarm) → VIP

```bash
# Set DNS search domain
docker run --dns-search example.com --dns 8.8.8.8 myapp
...
| Type | Description | Managed by | Persists after `docker rm` |
|------|-------------|------------|---------------------------|
| **Named volume** | Docker creates at `/var/lib/docker/volumes/<name>/` | Docker | Yes |
| **Anonymous volume** | Auto-generated name | Docker | No (removed with container if `--rm`) |
| **Bind mount** | Maps specific host path | User/OS | Yes (host filesystem) |
| **tmpfs mount** | In-memory, never written to disk | Docker | No (memory only) |
...
# Shows: /var/lib/docker/volumes/pgdata/_data
...
> [!CAUTION]
> Bind mounts expose the host filesystem. If the container runs as root and writes to a bind mount, it writes as root on the host. Always set appropriate `:ro` (read-only) flags where possible.

### Volume drivers

Volume drivers allow volumes to be backed by external storage systems:
...
# Using convoy, rex-ray, etc.
docker volume create --driver rexray/ebs --name myebs --opt size=20
```

### Volume permissions (common pain point)

When a volume is mounted into a container, the directory's UID/GID on the host determines who can write to it inside the container:

```dockerfile
# Fix: set ownership in Dockerfile before VOLUME declaration
...
Or at runtime:
```bash
# Init container pattern to set permissions
...
Docker drops most capabilities by default when running containers. The default allowed set is minimal — a container running as root inside still has far fewer privileges than host root.
...
| `SYS_PTRACE` | Debug processes (strace, gdb) |
| `SYS_ADMIN` | Wide range of admin ops (often abused) |
| `CHOWN` | Change file ownership |
| `DAC_OVERRIDE` | Bypass file permission checks |

### seccomp

Seccomp (secure computing mode) filters syscalls. Docker's default profile blocks ~44 syscalls, including `ptrace`, `kexec_load`, `mount`, and others that are dangerous in container contexts.
...
On Ubuntu/Debian, Docker applies the `docker-default` AppArmor profile to all containers. It restricts access to sensitive kernel features and filesystem paths.

```bash
# Check AppArmor profile on running container
docker inspect mycontainer | jq '.[0].HostConfig.SecurityOpt'

# Apply custom profile
apparmor_parser -r -W my-custom-profile
docker run --security-opt apparmor=my-custom-profile myapp
```

### User Namespaces (rootless)

User namespace remapping allows container root (UID 0) to map to an unprivileged UID on the host. Even if a container escape occurs, the attacker has no host privileges.

```bash
# Enable user namespace remapping in daemon
# /etc/docker/daemon.json
{
  "userns-remap": "default"
}

# "default" uses the "dockremap" system user
# Container UID 0 → host UID 100000
# Container UID 1000 → host UID 101000
```

**Rootless Docker:** Runs the entire Docker daemon as a non-root user. All container processes run as subprocesses of the unprivileged user. Requires `newuidmap`/`newgidmap` and `/etc/subuid` entries.
...
Prevents container processes from gaining new privileges via `setuid` binaries or file capabilities:
...
Combined with `--user 1000` and `--cap-drop ALL`, this is the most hardened configuration for stateless workloads.

### Image signing — Cosign / Notary v2

```bash
# Sign image with keyless signing (OIDC-based, in CI/CD)
cosign sign --yes myrepo/myapp:sha256-<digest>
...
# Generate SBOM
syft myrepo/myapp:latest -o spdx-json > sbom.spdx.json
# or
trivy image --format cyclonedx --output sbom.json myrepo/myapp:latest

# Attest SBOM to image
cosign attest --predicate sbom.spdx.json --type spdx myrepo/myapp:latest
```
...
| `on-failure[:N]` | Restart if exit code non-zero, up to N times |
| `always` | Always restart, including on daemon start |
| `unless-stopped` | Always restart, except if explicitly stopped |

```bash
docker run --restart unless-stopped --name web nginx
docker update --restart on-failure:3 mycontainer
```

**Signal handling best practices:**
- Container PID 1 must handle SIGTERM for graceful shutdown
- Use `exec` form of CMD/ENTRYPOINT to avoid shell wrapper that doesn't forward signals
- Add `tini` or `dumb-init` as PID 1 for proper zombie reaping

```dockerfile
# tini as PID 1 — handles signals and zombie reaping
FROM ubuntu:22.04
...
BuildKit is Docker's next-generation build backend (default since Docker 23.0).
...
docker build (client)
    │
    │ Frontends (Dockerfile parser, HCL, etc.)
    │
    ▼
BuildKit daemon (buildkitd)
    │
    ├── LLB (Low-Level Build) graph solver
    ...
    │
    └── Workers (runc, containerd)
```

LLB (Low-Level Build) is a DAG of operations. The solver executes the DAG with maximum parallelism — independent build stages execute concurrently.
...
FROM python:3.12 AS backend-builder     # runs in parallel with frontend-builder
...
```dockerfile
# apt cache persists between builds — no re-download
...
**Secret mounts (zero-trace secrets):**
```dockerfile
...
docker buildx build --load --platform linux/amd64 -t myapp:local .
```

**Platform emulation** uses QEMU registered via `binfmt_misc`:
```bash
