...
```
Azure Interview — Easy Topics
├── Azure DevOps
│   ├── Service Connections (SP vs Managed Identity vs OIDC)
│   ├── Microsoft-hosted vs self-hosted agents
│   ├── YAML pipeline structure (trigger, pool, stages, jobs, steps)
│   └── Environments + approval gates + deployment strategies
├── Identity & Security
│   ├── Managed Identity (system-assigned vs user-assigned)
│   ├── Service Principal (app registration + secret/cert)
│   ├── Azure Key Vault (secrets, keys, certs)
│   ├── RBAC (Owner, Contributor, Reader, custom roles)
│   └── Entra ID (tenant, users, groups, MFA)
├── Resource Management
│   ├── Resource Groups (logical containers, billing unit)
│   ├── ARM Templates vs Bicep (IaC for Azure resources)
│   ├── Tags (cost allocation, governance, automation)
│   └── Azure Subscriptions + Management Groups hierarchy
├── Compute
│   ├── Azure VMs (sizes, availability sets vs zones)
│   ├── App Service (PaaS, deployment slots for blue-green)
│   ├── Azure Kubernetes Service (AKS) basics
│   ├── ACR (Azure Container Registry) — managed image store
│   └── Azure Functions (serverless triggers/bindings)
├── Networking
│   ├── VNet + Subnets (isolation boundary)
│   ├── NSGs (stateful firewall at NIC/subnet level)
│   ├── VNet Peering (direct backbone link, non-transitive)
│   ├── Private Endpoints (private IP for PaaS services)
│   └── Azure Load Balancer vs Application Gateway basics
├── Storage
│   ├── Blob tiers (Hot / Cool / Archive)
│   ├── Storage Account redundancy (LRS/GRS/ZRS/GZRS)
│   └── SAS tokens vs Managed Identity access
├── AKS Networking
│   ├── Kubenet vs Azure CNI vs Azure CNI Overlay
│   └── Node pools (system vs user pool types)
├── Monitoring
│   ├── Azure Monitor (metrics + logs unified platform)
│   ├── Log Analytics Workspace (KQL queries)
│   └── Application Insights (APM for apps)
└── Fundamentals (100-Question Set)
    ├── Cloud concepts (CapEx vs OpEx, SLA, availability)
    ├── Compute, Storage, Networking, Security basics
    ├── Containers (ACR, AKS basics)
    └── IaC (ARM, Bicep, Terraform intro)
```
...
```
Azure Interview — Medium Topics
├── Identity & Credentials
│   ├── Managed Identity vs Service Principal (lifecycle, rotation, use cases)
│   ├── Workload Identity Federation (OIDC token exchange, no static secrets)
│   └── ADO Service Connections vs GitHub OIDC (federated vs SP-based)
├── IaC & Deployment
│   ├── Terraform state backend (azurerm: blob container + state locking)
│   ├── Bicep vs ARM (DSL vs JSON, modules, what-if)
│   └── Blue-Green deployments (App Service deployment slots)
├── AKS Deep Dive
│   ├── Azure CNI vs Azure CNI Overlay vs Kubenet (IP allocation trade-offs)
│   ├── AKS upgrade order (control plane → system pool → user pools)
│   ├── Blue-green node pool strategy (parallel pool, cordon/drain, delete old)
│   ├── AGIC vs nginx-ingress (Azure-native vs community, WAF integration)
│   └── AKS cost management (Kubecost, right-sizing, spot node pools)
├── Key Vault Integration
│   ├── CSI Secret Store Driver (mount KV secrets as volumes)
│   ├── RBAC vs Access Policies (prefer RBAC, unified authorization)
│   └── Private endpoint for KV (disable public access in production)
├── Networking
│   ├── VNet Peering (non-transitive, CIDR no overlap, global peering)
│   ├── Private DNS Resolver (hybrid DNS forwarding, inbound/outbound endpoints)
│   └── Azure Firewall vs NVA (managed vs BYO)
├── CI/CD Patterns
│   ├── Multi-stage ADO pipeline (YAML: Build → Staging → Production)
│   ├── ADO Environments + checks (manual approval, branch policies)
│   ├── Logic Apps vs Functions (workflow vs code, stateful vs stateless)
│   └── Container Apps vs AKS (managed abstraction vs full control)
├── Governance
│   ├── Azure Policy enforcement (Deny at subscription/MG level)
│   ├── AKS security baseline policy (restrict privileged pods, enforce probes)
│   └── DeployIfNotExists (auto-remediation for non-compliant resources)
├── Observability
│   ├── Azure Managed Prometheus + Grafana (AKS metrics)
│   └── Log Analytics KQL queries (diagnostic logs, container logs)
└── 100 Medium Questions
    ├── Governance, Compute, Networking, Storage
    ├── Containers, Monitoring, IaC, Security
    └── Data, Serverless, Cost optimization
```
...
```
Azure Interview — Hard Topics
├── Enterprise Networking Architecture
│   ├── Hub-and-Spoke for AKS (Azure Firewall, UDRs, egress control)
│   ├── Private AKS cluster (private API server endpoint, no public IP)
│   ├── Azure Firewall FQDN rules for AKS egress
│   └── Forced tunneling (outbound traffic through on-prem firewall)
├── Multi-Tenant CI/CD Governance
│   ├── Per-team service connections with scoped permissions
│   ├── OIDC-based service connections (no secrets)
│   ├── Azure Policy on pipelines (enforce tagging, regions)
│   └── Audit trail: ADO pipeline logs + CloudAudit
├── Load Balancing Deep Dive
│   ├── Front Door vs Application Gateway (when to use both)
│   ├── Front Door: global CDN + WAF + failover routing
│   ├── App Gateway: regional L7 WAF + path-based routing
│   └── Use-both pattern: FD (global) → App GW (regional) → AKS
├── Private DNS Failure Diagnosis
│   ├── Private DNS zone not linked to VNet → resolution fails
│   ├── Custom DNS server overrides Azure DNS resolver
│   ├── Private DNS Resolver (inbound/outbound endpoints)
│   └── DNS forwarder rules for hybrid resolution
├── Zero-Trust AKS Architecture
│   ├── Network Policies (Calico / Azure NPM)
│   ├── Workload Identity (no pod SA static credentials)
│   ├── mTLS with Istio / SPIFFE/SPIRE
│   ├── Admission control (OPA Gatekeeper / Kyverno)
│   └── Image signing (Notation + Ratify)
├── Multi-Region HA
...
│   └── RTO/RPO trade-offs per tier
├── Azure Landing Zones
│   ├── Management Group hierarchy design
│   ├── Policy assignments at MG level (top-down)
│   ├── Platform vs Application subscriptions
│   └── Connectivity subscription (hub VNet, ExpressRoute)
├── AKS Isolation Patterns
│   ├── Node pool taints/tolerations (per team)
│   ├── Namespace isolation (ResourceQuota, LimitRange)
│   ├── RBAC scoped to namespace
│   └── NetworkPolicy (deny-all default, allow selectively)
├── Data Analytics Architecture
│   ├── Azure Data Factory (orchestration/ETL)
│   ├── Synapse Analytics (unified analytics)
│   ├── Databricks (Spark-based ML + data engineering)
│   └── Delta Lake on ADLS Gen2
├── Hybrid Connectivity
│   ├── ExpressRoute (dedicated circuit, SLA, BGP)
│   ├── VPN Gateway (IPSec over internet, backup to ER)
│   └── ExpressRoute + VPN coexistence (failover)
├── AKS Outbound Types
│   ├── loadBalancer (default, managed public LB)
│   ├── userDefinedRouting (UDR → Azure Firewall)
│   └── managedNATGateway / userAssignedNATGateway
└── AKS DNS Troubleshooting
    ├── CoreDNS config (custom forwarders)
    ├── ndots:5 setting causes excessive DNS lookups
    └── Private cluster: DNS resolution to private API server
```
...
- [Cloud Services Overview](../12_Azure/README.md)
- [Enterprise Landing Zones](../12_Azure/notes/azure-aks-and-devops.md)
- [Azure to AWS Similarities](../12_Azure/notes/notes.md)
- [Azure Interview Questions (Easy)](../12_Azure/interview.md) | [Medium](../12_Azure/interview.md) | [Hard](../12_Azure/interview.md)
...
- [Terraform Fundamentals](../10_Terraform/README.md)
- [Advanced Terraform Patterns](../10_Terraform/notes/terraform-backends-and-state.md)
...
- [Jenkins & CI/CD Pipelines Overview](../06_Jenkins/README.md)
- [Platform Engineering for CI/CD](../06_Jenkins/notes/platform-engineering-for-cicd.md)
...
- [Containers and Orchestration Overview](../05_Kubernetes/notes/containers-orchestration-README.md)
- [Docker & Runtimes Security](../04_Docker/notes/container-runtimes-and-security.md)
- [Enterprise K8s Architecture](../05_Kubernetes/notes/enterprise-kubernetes-architecture.md)
...
- [Networking Overview](../02_Networking/README.md)
- [Enterprise Networking & Protocols](../02_Networking/notes/protocols.md)
- [Advanced K8s Networking](../05_Kubernetes/notes/k8s-networking.md)
...
- [Monitoring & Observability Fundamentals](../15_Observability_and_SRE/README.md)
...
- [Supply Chain Security (SLSA)](../14_DevSecOps/notes/supply-chain-security-and-slsa.md)
- [IaC Policy & GitOps](../10_Terraform/README.md)
...
- [Scripting Overview](../01_Linux_and_Scripting/README.md)
- [Engineering Automation at Scale](../01_Linux_and_Scripting/notes/shell-scripting.md)
...
- [Linux OS Fundamentals](../01_Linux_and_Scripting/README.md)
- [Advanced Linux Performance & Hardening](../01_Linux_and_Scripting/notes/advanced-linux.md)
- [Incident Response Protocol](../15_Observability_and_SRE/notes/troubleshooting_guide.md)
...
- [Azure Scenario Drills](../12_Azure/scenarios.md)
- [General Interview Questions](../07_Interview_Preparation/general-interview-questions.md)
- [Interview Questions (Easy)](../07_Interview_Preparation/interview-questions-easy.md) | [Medium](../07_Interview_Preparation/interview-questions-medium.md) | [Hard](../07_Interview_Preparation/interview-questions-hard.md)
