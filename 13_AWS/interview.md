...
```
AWS Interview — Easy Topics
├── IAM Fundamentals
│   ├── IAM User (permanent, long-term credentials) vs IAM Role (temporary, assumed)
...
│   └── S3 bucket policy (JSON, conditions) vs S3 ACL (legacy, predefined grants)
├── Networking Basics
│   ├── Availability Zones (isolated datacenters within a region)
│   ├── VPC (isolated virtual network, solves flat-public-network problem)
│   ├── Public subnet (route to IGW) vs Private subnet (route to NAT GW)
│   └── AWS Regions (latency, data residency, compliance, service availability, cost)
├── Scaling & Compute
│   ├── Horizontal scaling (add instances, ASG) vs Vertical scaling (resize instance)
│   └── EC2 instance metadata (169.254.169.254, IMDSv2 mitigates SSRF)
├── Storage
│   ├── S3 storage classes: Standard / Standard-IA / One Zone-IA / Glacier Instant / Glacier Flexible / Deep Archive
│   └── S3 Lifecycle policies (auto-transition between classes as objects age)
├── Observability
│   └── CloudWatch: metrics, logs, alarms, dashboards, Events/EventBridge
└── 100 Easy Questions
    ├── Cloud Fundamentals (CapEx vs OpEx, SLA, global infra)
    ├── Compute (EC2 types, AMI, instance store vs EBS)
    ├── Storage (S3, EBS, EFS basics)
    ├── Networking (VPC, SG, NACL, route tables)
    ├── Containers (ECS, EKS basics)
    ├── Database (RDS, DynamoDB basics)
    ├── Security (IAM, encryption at rest/transit)
    ├── Deployment & Management (CloudFormation, Systems Manager)
...
```
AWS Interview — Medium Topics
├── IAM Deep Dive
│   ├── Policy evaluation order: Explicit Deny → SCP → Resource-based Allow → Identity-based Allow → Implicit Deny
│   ├── SCPs (Service Control Policies) — ceiling, not grants; even root can't exceed SCP
│   └── Permissions boundaries — max IAM permissions for a principal
├── EKS & Containers
│   ├── IRSA (IAM Roles for Service Accounts): OIDC issuer → IAM trust → SA annotation → token webhook → AssumeRoleWithWebIdentity
│   ├── ECS vs EKS comparison (control plane, networking, cost, ecosystem, portability)
│   └── ECS awsvpc mode (each task gets its own ENI + SG)
├── Networking
│   ├── VPC Peering (one-to-one, non-transitive, CIDR no overlap, 2-5 VPCs)
│   ├── Transit Gateway (hub-and-spoke, transitive, thousands of VPCs, inter-region)
│   ├── ALB (L7, content-based routing, WebSocket, gRPC) vs NLB (L4, static IP, ultra-high throughput)
│   └── SSM Session Manager vs SSH (no port 22, no key mgmt, IAM-controlled, private subnet works)
├── Serverless
│   ├── Lambda execution model (Firecracker microVM, cold start 100ms-1s+)
│   ├── Cold start mitigations (Provisioned Concurrency, ARM64, smaller package, init outside handler)
│   └── Lambda concurrency: reserved vs provisioned vs account limit
├── Auto Scaling
│   ├── Target tracking (maintain metric at target — recommended)
│   ├── Step scaling (tiered response at multiple thresholds)
│   └── Scheduled scaling (pre-scale for known traffic spikes)
└── 100 Medium Questions
    ├── IAM: cross-account access, resource-based policies, trust policies
    ├── Networking: VPC endpoints, flow logs, PrivateLink
    ├── EC2: placement groups, instance store vs EBS, ASG lifecycle hooks
    ├── Storage: S3 CRR, MFA Delete, S3 Object Lock
    ├── Containers: ECS task definitions, EKS managed node groups
    ├── Databases: RDS Multi-AZ vs Read Replicas, Aurora Serverless
    ├── CI/CD: CodePipeline, CodeBuild, deployment strategies
    ├── Monitoring: CloudWatch Logs Insights, X-Ray, Container Insights
    ├── Security: GuardDuty, Security Hub, WAF, Shield
    └── Serverless: API Gateway, SQS, SNS, EventBridge patterns
```
...
```
AWS Interview — Hard Topics
├── Spot Instance EKS Design
│   ├── Karpenter with diversified NodePools (multiple instance families + sizes)
│   ├── Node Termination Handler (drain on spot interruption notice)
│   ├── PodDisruptionBudgets (protect critical pods during drain)
│   ├── On-demand baseline for system/critical workloads
│   └── Savings Plans for predictable on-demand baseline
├── Multi-Account Organization Design
│   ├── Account structure: Management, Security OU, Infrastructure OU, Workloads OU
│   ├── SCPs at OU level (deny unapproved regions, protect foundation roles)
│   ├── AWS Control Tower (landing zone automation, guardrails)
│   ├── Centralized security (Log Archive + Security Tooling accounts)
│   └── Cross-account roles (OrganizationAccountAccessRole pattern)
├── Lambda Cold Start Diagnosis & Fix
│   ├── X-Ray tracing to identify initialization duration
│   ├── Provisioned Concurrency (pre-warm N environments, costs money)
│   ├── Memory tuning (more memory → more CPU → faster init)
│   ├── SnapStart for Java (pre-snapshot initialized state)
│   └── Package size reduction (fewer dependencies → faster download)
├── Multi-Region DR (RTO < 15 min)
│   ├── Aurora Global Database (cross-region replica, <1s replication lag)
│   ├── IaC (Terraform) for all infrastructure (re-deploy in minutes)
│   ├── Route 53 health checks + failover routing policy
│   ├── Pre-warmed capacity in DR region (Pilot Light or Warm Standby)
│   ├── Automated runbook (Lambda or Step Functions for failover)
│   └── Regular DR drills (test failover quarterly)
└── 100 Hard Questions
    ├── IAM: complex policy evaluation, cross-account trust chains
    ├── Networking: Transit Gateway routing, PrivateLink, Direct Connect
    ├── EKS: IRSA deep dive, VPC CNI prefix delegation, Karpenter
    ├── Security: GuardDuty findings, SCPs, permission boundaries
    └── Cost: RI/SP optimization, Karpenter right-sizing, FinOps
```
